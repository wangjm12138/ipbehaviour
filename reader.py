...
class IPHandle(object):
    ...
    def find_geoip(cls,ip=None,dbinsert=True,dbsearch=True,tbsearch=True):
        ...
        if (dbsearch == False or dberror or dbselect_result == None or len(dbselect_result) == 0) and tbsearch:
            while retry < 5:
                try:
    ...
    def find_userip(cls,ip=None,url=None,certId=None,certKey=None,\
        ...
        if all((url,certId,certKey)) == False and config!=None and config.noconf == False:
            url = config.url
    ...
    def feature_engine(self):
        ...
        if len(df) == 0:
           raise ValueError("http_log_content is empty!")
    ...
    def read_http_log(self,flag=True):
        ...
        with open(self.http_log) as f:
            for line in f:
                ...
                timestamp = line_item[0]
                protocol = line_item[1]
                method = line_item[2]
                host = line_item[3]
                params = line_item[4]
                status = line_item[5]
                size = line_item[6]
                src_ip = line_item[7]
                src_port = line_item[8]
                dest_ip = line_item[9]
                dest_port = line_item[-1]
